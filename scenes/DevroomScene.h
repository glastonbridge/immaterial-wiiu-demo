...
struct DevroomScene : public SceneBase {
  ...
  void update(double time) final {
    ...
    int gridSize = int(sqrtf(_ID_ASSETS_MAX) + 1);
    for(int i = 0; i < _ID_ASSETS_MAX; i++) {
        int x = i % gridSize;
        int z = i / gridSize;
        instances[i].transform = glm::translate(glm::mat4(1.f), glm::vec3(x * gridSpacing - (gridSize / 2.0) * gridSpacing, 0.f, z * gridSpacing - (gridSize / 2.0) * gridSpacing));
        instances[i].transform = glm::rotate(instances[i].transform, rotVal, glm::vec3(0.f, 1.f, 0.f));
