...
void EepyScene::setup() {
  ...
  instances.back().transform =
      glm::scale(glm::translate(glm::mat4(1.f), glm::vec3(20.f, 0.f, 10.f)),
                 glm::vec3(2.5f)) *
      rot90;

  instances.emplace_back(ID_pillow);
  instances.back().transform =
      glm::scale(glm::translate(glm::mat4(1.f), glm::vec3(20.f, 0.f, -7.f)),
                 glm::vec3(2.5f)) *
      rot90;
}
...
void EepyScene::update(double time) {
  ...
  auto const t1 =
      std::min(syncVal("Train:Car1"), float(std::size(eepyTrack) - 1));
