...
// Bail if not the right name
    ...
    if (node->mesh) {
      ...
      for (size_t faceIdx = 0; faceIdx < mesh->faces.count; faceIdx++) {
        ...
        for (size_t vertIdx = faceFirstVertIdx; vertIdx < faceFirstVertIdx + 3;
             vertIdx++) {
#ifdef DEBUG
...
#endif
          ufbx_vec3 normal =
              ufbx_get_vertex_vec3(&mesh->vertex_normal, vertIdx);
#ifdef DEBUG
          ...
          // Them bones. Of which we support two. More are possible at increased
          // cost.
          ufbx_vec2 boneIdx;
          ufbx_vec2 boneWgt;
          ...
          if (skin != NULL) {
            ufbx_skin_vertex *vertSkinData =
                &skin->vertices.data[mesh->vertex_indices.data[vertIdx]];
            ...
            if (numBones >= 1) {
              boneIdx.x =
                  skin->weights.data[vertSkinData->weight_begin].cluster_index;
            ...
            if (numBones >= 2) {
              boneIdx.y = skin->weights.data[vertSkinData->weight_begin + 1]
                              .cluster_index;
              boneWgt.y =
                  skin->weights.data[vertSkinData->weight_begin + 1].weight;
#ifdef DEBUG
          ...
          boneIndices.push_back(
              boneIdx.x); // up to 4 bones per vert possible using this method.
      ...
      if (scene->anim_stacks.count >= 1 && skin != NULL) {
        if (scene->anim_stacks.count > 1) {
          WHBLogPrintf("Warning: more than one animation stack (%ld), only the "
                       "longest will be used",
        ...
        for (int i = scene->anim_stacks.count - 1; i >= 0; i--) {
          ...
          int frames =
              (int)((anim_stack->anim.time_end - anim_stack->anim.time_begin) /
                        (1.0 / 60.0) +
                    0.5);
...
// We sample at 60fps
        ...
        size_t frameCount =
            (size_t)((anim_stack->anim.time_end - anim_stack->anim.time_begin) /
                         frameDur +
                     0.5);
        ...
        for (size_t frame = 0; frame < frameCount; frame++) {
          ...
          for (size_t boneIdx = 0; boneIdx < boneCount; boneIdx++) {
            ...
            if (!havePrinted) {
#ifdef DEBUG
              WHBLogPrintf("  * bone %ld: %s", boneIdx,
            ...
            ufbx_transform transform =
                ufbx_evaluate_transform(&anim_stack->anim, bone, frameTime);
            ufbx_matrix transformMatLocal =
                ufbx_transform_to_matrix(&transform);
            ufbx_matrix transformMat =
                ufbx_matrix_mul(&transformMatLocal, &cluster->geometry_to_bone);

#ifdef DEBUG
            WHBLogPrintf(
                "%f %f %f\n%f %f %f\n%f %f %f\n%f %f %f\n\n", transformMat.v[0],
                transformMat.v[1], transformMat.v[2], transformMat.v[3],
                transformMat.v[4], transformMat.v[5], transformMat.v[6],
                transformMat.v[7], transformMat.v[8], transformMat.v[9],
                transformMat.v[10], transformMat.v[11]);
#endif
            ...
            frameMats.push_back(glm::mat4(
                ...
                transformMat.v[9], transformMat.v[10], transformMat.v[11],
                1.0));
          }
        ...
        WHBLogPrintf(
            "Yikes! Invalid animation stack count (= %ld) or no skin present!",
            scene->anim_stacks.count);
      }
    } else {
  ...
  WHBLogPrintf("loaded %i vertex elements, %i texcoord elements and %i "
               "normals, %d animation frames",
