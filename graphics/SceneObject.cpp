...
LoadText(SceneMaterial * material, std::string text, std::vector<std::vector<float>> vertices, std::vector<std::vector<float>> normals)
{
   std::unique_ptr<SceneObjectImpl> _impl;
   _impl.reset(new SceneObjectImpl());
   ...
   _impl->setMaterial(material);
   _impl->getRenderObject()->setAttribBuffer(BufferType::VERTEX, stringVertices.data(), 4 * 3, stringVertices.size() / 3);
   _impl->getRenderObject()->setAttribBuffer(BufferType::COLOR, stringVertexColors.data(), 4 * 4, stringVertexColors.size() / 4);
   _impl->getRenderObject()->setAttribBuffer(BufferType::TEXCOORD, stringTexCoords.data(), 4 * 2, stringTexCoords.size() / 2);
   _impl->getRenderObject()->setAttribBuffer(BufferType::NORMAL, stringNormals.data(), 4 * 3, stringNormals.size() / 3);
