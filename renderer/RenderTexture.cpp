...
RenderTexture::RenderTexture(const std::string &path, bool isCubeMap) {
  ...
  if (!isCubeMap) {
    unsigned error =
        lodepng::decode(image, width, height, WIIU_PATH_PREFIX + path);

    WHBLogPrintf("Setting up a texture from %s: %i x %i", path.c_str(), width,
                 height);
    if (error) {
      WHBLogPrintf("PNG load error on %s: %u: %s", path.c_str(), error,
    ...
    for (int i = 0; i < 6; i++) {
      ...
      image_load.clear();
      unsigned error = lodepng::decode(image_load, width, height,
                                       WIIU_PATH_PREFIX + cubeMapPath);
      WHBLogPrintf("Setting up a cube face texture from %s: %i x %i",
                   path.c_str(), width, height);
      if (error) {
        WHBLogPrintf("PNG load error on %s: %u: %s", cubeMapPath.c_str(), error,
                     lodepng_error_text(error));
      }
  ...
  texture.surface.tileMode =
      GX2_TILE_MODE_DEFAULT; // GX2_TILE_MODE_LINEAR_ALIGNED; // probably memory
                             // layout stuff?
  ...
  if (tempSurface.pitch != width) {
    WHBLogPrintf("Converting pitch and copying %i bytes to texture",
    ...
    for (int i = 0; i < tempSurface.height; i++) {
      memcpy((uint32_t *)&texDataPtrPitched[i * tempSurface.pitch],
             (uint32_t *)&imageDataPtrContinuous[i * width],
             width * sizeof(uint32_t));
    }
  } else {
  ...
  GX2Invalidate(GX2_INVALIDATE_MODE_CPU_TEXTURE, tempSurface.image,
...
void RenderTexture::renderUsing(const WHBGfxShaderGroup *group, int binding) {
  if (binding < group->pixelShader->samplerVarCount) {
    // WHBLogPrintf("Binding a %p texture for group %p @ %i -> %i / %i", this,
    // group, binding, group->pixelShader->samplerVars[binding].location,
    // group->pixelShader->samplerVarCount);
